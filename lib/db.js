...
function utcNow () {
  // returns '2016-03-08T18:43:11.471Z'
...
module.exports.createDataset = function (dataset, meta, callback) {
  var baseMeta = {
    dateCreated: utcNow(),
    dateUpdated: {
      pits: null,
...
module.exports.updateDataset = function (dataset, callback) {
  this.getDataset(dataset.id, (err, dataset, meta) => {
    if (err) {
      ...
      this.createDataset(dataset, meta, callback)
    }
  })
}
...
module.exports.getDatasets = function (callback) {
  H(db.createReadStream())
    ...
    .pluck('value')
    // .map((value) => addDateUpdated(value.dataset, value.meta))
